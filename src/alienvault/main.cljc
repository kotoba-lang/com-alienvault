(ns alienvault.main
  "Kotodama WASM entrypoint for the Alienvault clean-room actor (L4) — Clojure port.
  ...
  facts (`alienvault.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "alienvault")
(def tier "L4")
...
(defn page-limit
  ...
  src/alienvault/page_limit.kotoba."
  [requested]
  (if (pos? requested)
...
(defn emit-facts
  "EAVT facts for one record: {\"alienvault.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "alienvault-compat" :tier tier :entities entities} 200])
...
(defn start! [] :alienvault-compat/ready)
